...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    # 我不懂这个 知识  是逐个判断所有的都平衡就是平衡吗，有没有不平衡被后面匀平衡的情况， 而且这里不考虑进来的是胳膊似乎
    if is_planet(m):
        return True
    else:
        m_left, m_right = end(left(m)), end(right(m))
        return total_mass(m_left) * length(left(m)) == total_mass(m_right) * length(right(m)) and balanced(m_left) and balanced(m_right)
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        #     path_amount.append(sum(first+[max_path_sum(branch)]))
        # print('DEBUG',path_amount)
        # return max(path_amount)
        # 这上面是我的解
    ...
    像是在每一层寻找最大的标签，当寻找到倒数第二层是，返回当前label和下层叶子label，
    再返回到倒数第三层中，得出最大值与label结合，依次返回得到最大
    '''

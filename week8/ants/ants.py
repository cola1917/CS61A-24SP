...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not attack_place.is_hive:
            # print('DEBUG',self.place)
            targets.append(random_bee(attack_place.bees))
            attack_place = attack_place.entrance
        # print('DEBUG',targets)
        # for target in targets:
        #     if target:
        #         # print('DEBUG',self.upper_bound - 1)
        #         if targets.index(target) >= self.lower_bound and targets.index(target) <= self.upper_bound:
        #             return target
        # 看起来应该比上面快
        for i in range(len(targets)):
            if targets[i]:
                # print('DEBUG',self.upper_bound - 1)
                if i >= self.lower_bound and i <= self.upper_bound:
                    return targets[i]
        return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        print('DEBUG',self.place)
        self.damage = self.damage + amount if amount >= self.health else amount
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        return self.is_container and not other.is_container and not self.ant_contained
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        print('DEBUG', next_place.ant)
        while next_place:
            print('DEBUG', next_place.ant)
            if next_place.ant:
                if not next_place.ant.is_double:
                    next_place.ant.damage *=2
                    next_place.ant.is_double = True
                if next_place.ant.is_container:
                    if next_place.ant.ant_contained:
                        if not next_place.ant.ant_contained.is_double:
                            next_place.ant.ant_contained.damage *= 2
                            next_place.ant.ant_contained.is_double = True
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        result = {}
        next_palce = self.place
        distance = 0
        if next_palce.ant.is_container:
            result[next_palce.ant] = distance
        while not next_palce.is_hive:
            # print('DEBUG',next_palce)
            # ant
            # 有蚂蚁？
            if next_palce.ant:
                # 是容器？
                if next_palce.ant.is_container:
                    # 先放容器进去 ，因为容器肯定要放
                    result[next_palce.ant] = distance
                    # 不是容器判断是自己吗，也就是是激光蚂蚁吗
                    if next_palce.ant.ant_contained != self:
                        result[next_palce.ant.ant_contained] = distance
                # 不是容器？  判断下是自己吗
                elif next_palce.ant != self:
                    result[next_palce.ant] = distance
            # bees
            for bee in next_palce.bees:
                result[bee] = distance
            distance += 1
            next_palce = next_palce.entrance
        print('DEBUG',result)
        return result
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        ...
        result = self.damage - distance * 0.25 - self.insects_shot / 16
